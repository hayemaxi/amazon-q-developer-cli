...
impl ContextManager {
    ...
    pub async fn run_conversation_start_hooks(&self, updates: &mut impl Write) -> Vec<(String, Result<String>)> {
        ...
        hooks.extend(self.profile_config.hooks.conversation_start.iter());
    ...
    pub async fn run_per_prompt_hooks(&self, updates: &mut impl Write) -> Vec<(String, Result<String>)> {
        ...
        hooks.extend(self.profile_config.hooks.per_prompt.iter());

        self.hook_executor.run_hooks(hooks, false, updates).await
    }
}
