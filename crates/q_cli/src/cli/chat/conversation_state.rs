...
impl ConversationState {
    ...
    pub fn abandon_tool_use(&mut self, tools_to_be_abandoned: Vec<QueuedTool>, mut deny_input: String) {
        debug_assert!(self.next_message.is_none());
        ...
        // Give the assisstant a more contextual response
        if ["n", "N"].contains(&deny_input.as_str()) {
            deny_input = "Nevermind.".to_string();
        }
        let msg = UserInputMessage {
            content: deny_input,
            user_input_message_context: Some(user_input_message_context),
